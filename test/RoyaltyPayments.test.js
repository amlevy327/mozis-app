...
contract('RoyaltyPayments', ([owner, mozis, artist, tester1]) => {
  ...
  describe('royalty payments', () => {
    it('tracks payments and releases', async () => {
      ...
      totalReleased = await royaltyPayments.totalReleased()
      totalReleased.toString().should.equal('0', 'initial total released is correct')
      
      await royaltyPayments.release(artist)
      let artistReleased = await royaltyPayments.released(artist)
      ...
      await royaltyPayments.release(mozis)
      let mozisReleased = await royaltyPayments.released(mozis)
